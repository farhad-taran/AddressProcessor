...
namespace AddressProcessing.CSV
{
    /*
            ...
            *) the class does not implement an interface and since its operation access external documents to the process it will not be able to be mocked or stubbed in unit tests for classes that use this client.
            * you dont necessarily have to extract an interface, you can create an abstract class or subclass this class and use the new keyword on the inherited members to over ride their behaviour in the stub subclass.

            *) although C# is pass by reference, it treats strings in a special way so this method public bool Read(string column1, string column2) will never assign and return the correct CSV values.
            ...
            *)there is temporal coupling on the close method and on the open method. meaning the caller has to always know that it should first call the open method and when it is done, call this method once they are finished with the class because this method uses streams that need to be disposed.
            * this is a continution of the bad design decision made by not implementing the IDisposable interface. Implementing the IDisposable interface also gives a good indicator to the client that this class has
            ...
            *)the class breaks the single responsibility principle, it provides functionality for writing and reading to a file using a Mode flag.
            ...
            *)I can see that the _readerStream.Read and _writerStream.Write are abstracted inside two functions, in the current implementation this seems like extra unnecessary complexity as this does not provide any benefit 
